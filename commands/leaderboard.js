...
module.exports = {
  ...
  execute: async (client, interaction) => {
    ...
    let collection = await model.find({ 'xp.id': interaction.guildId }, { 'xp.$': 1, '_id': 1 });
    if (collection instanceof Error) return interaction.reply({ content: `An error was encountered: ${collection.message}`, ephemeral: true });
    if (!collection.length) return interaction.reply({ content: 'This server has no leaderboard yet. Start chatting to gain XP!', ephemeral: true });
    ...
    const fetched = await interaction.guild.members.fetch({ user: collection.map(x => x._id )})
    .catch(error => error);

    if (fetched instanceof Error) return interaction.reply({ content: `An error was encountered: ${fetched.message}`, ephemeral: true });
    const mappableCollection = collection.filter(x => fetched.has(x._id)).sort((A,B) => B.xp[0].xp - A.xp[0].xp).splice(0,10);
    ...
    for (const [index, data] of Object.entries(mappableCollection)){
      ...
      files.push(new MessageAttachment(canvas.toBuffer(), `lb-${data._id}.png`));
    };

    const fullList = collection.filter(x => fetched.has(x._id)).sort((A,B) => B.xp[0].xp - A.xp[0].xp).map((x,i) => {
    ...
    return interaction[interaction.deferred ? 'editReply' : 'reply']({ files, components, embeds: mappableCollection.map((data, index) =>
      new MessageEmbed()
