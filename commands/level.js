    ...
    execute: async (client, interaction) => {
        ...
        if (user.bot)
            return interaction.reply({
                ephemeral: true,
                content: '⚠️ Bots cannot earn xp!'
            });

        const subdocument_template = {
            _id: user.id,
            xp: [{
                  xp: 0,
        ...
        let collection = await model.find({
        ...
        if (!collection.some(x => x._id === user.id))
            cillectoion.push(subdocument_template);

        const document = collection
            .find(x => x._id === user.id);
        ...
        const presence = member.presence?.activities
            ...
            .splice(0,1)
            .map(x => `${emojis[x.type]} ${x.type[0]}${x.type.substr(1,10).toLowerCase()} ${x.name}`)[0];
        ...
        let wallpaper;
        try {
            wallpaper = await loadImage(document.wallpaper || '')
        } catch {
        ...
        const checkbox = await loadImage(join(
            __dirname,
            '../assets/images/icon/checkbox.png'
        ));

        const nemuicon = await loadImage(join(
            __dirname,
            '../assets/images/icon/nemu-icon-1.png'
        ));
        ...
        ctx.drawImage(
            ...
            (canvas.height * 0.2) - avatarRadius,
            avatarRadius * 2,
            avatarRadius * 2
        );
        ctx.restore();
        ...
        createVerifiedBadge(
            (canvas.width / 2) + avatarRadius * 0.8,
            (canvas.height * 0.2) + avatarRadius * 0.6,
            avatarRadius * 0.24
        );
        ...
        centerText(
            user.username,
            '18px Aller, "Segoe UI", "Unifont"', 'rgb(125,128,151)',
            '#' + user.discriminator,
            '15px Aller', 'rgb(126,148,223)',
            canvas.height * 0.42,
            canvas
        );
        ...
        centerText(
            xp_current.toLocaleString('en-US',{
                notation: 'compact',
                maximumFractionDigits: 2
            }),
            '25px Aller',
            xpgradient,
            '/' + xp_currmax.toLocaleString('en-US',{
                notation: 'compact',
                maximumFractionDigits: 2
            }),
            '15px Segoe UI',
        ...
        const {
            width: levelwidth,
            actualBoundingBoxAscent: levelAscent,
            ctualBoundingBoxDescent: levelDescent
        } = ctx.measureText(subdocument.level);

        ctx.fillStyle = gradientStyle(
            (canvas.width * 0.25) - (levelwidth / 2),
            480 - levelAscent,
            (canvas.width * 0.25) + (levelwidth/2),
            480 - levelAscent
        );
        ...
        // Rank Number
        ctx.font = '25px Aller';
        const rank = ordinalize(
            collection
                .sort((A,B) => B.xp[0].xp - A.xp[0].xp)
                .findIndex(x => x._id === user.id) + 1
        );

        const {
            width: rankwidth,
            actualBoundingBoxAscent: rankAscent,
            actualBoundingBoxDescent: rankDescent
        } = ctx.measureText(rank);

        ctx.fillStyle = gradientStyle(
            (canvas.width * 0.75) - (rankwidth/2),
            460 - rankAscent,
            (canvas.width * 0.75) + (rankwidth/2),
            460 - rankAscent
        );
        ...
        function createProgressBar(x, y, width, height, percent, style){ctx.beginPath();ctx.lineCap='butt';ctx.lineWidth=2;ctx.strokeStyle=style;ctx.moveTo(x,y);ctx.arcTo(x,y-(height/2),x+(height/2),y-(height/2),height/2);ctx.lineTo(x+width-(height/2),y-height/2);ctx.arcTo(x+width,y-(height/2),x+width,y,height/2);ctx.arcTo(x+width,y+(height/2),x+width-(height/2),y+(height/2),height/2);ctx.lineTo(x+(height/2),y+(height/2));ctx.arcTo(x,y+(height/2),x,y,height/2);ctx.stroke();ctx.save();ctx.clip();ctx.beginPath();ctx.lineWidth=height;ctx.moveTo(x,y);ctx.lineTo(x+(width*percent),y);ctx.stroke();ctx.restore();};
