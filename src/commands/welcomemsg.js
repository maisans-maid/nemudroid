...
module.exports = {
  ...
  execute: async (client, interaction) => {
    ...
    let profile = await client.database.models.guildSchema.findById(interaction.guildId).catch(() => {});
    if (!profile){
      profile = new client.database.models.guildSchema({_id: interaction.guildId});
    };
    if (profile instanceof Error){
      profile = client.localcache.serverprofiles[interaction.guildId];
    };
    if (!profile){
